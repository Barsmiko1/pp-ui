...
export default function SellOrdersPage() {
  ...
  const handleReleaseAssets = async () => {
    ...
    try {
      await orderApi.releaseAssets(selectedOrder.id)

      toast({
        title: "Assets released",
        description: `Assets for order ${selectedOrder.id} have been released to the buyer.`,
      })
      ...
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : "Failed to release assets",
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center justify-between space-y-2">
        <h2 className="text-3xl font-bold tracking-tight">Sell Orders</h2>
        <div className="flex items-center space-x-2">
          <Button onClick={() => router.push("/dashboard/new-order?type=sell")}>
        ...
        <TabsContent value={activeTab} className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage your cryptocurrency sell orders</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <EmptyState
                  ...
                  description={`You don't have any ${activeTab === "all" ? "" : activeTab} sell orders yet.`}
                  icon="arrowUp"
                  actionLabel="Create Sell Order"
                  onAction={() => router.push("/dashboard/new-order?type=sell")}
                />
              ) : (
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {filteredOrders.map((order) => (
                      <TableRow key={order.id}>
                        ...
                        <TableCell>{order.targetNickName}</TableCell>
          ...
          {selectedOrder && (
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="buyer" value={selectedOrder.targetNickName} className="col-span-3" readOnly />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
              ...
              {selectedOrder.status === 20 && ( // PAID
                <div className="flex justify-end space-x-2 pt-4">
                  <Button variant="outline" onClick={() => setIsDialogOpen(false)}>
                    Cancel
                  </Button>
                  <Button onClick={handleReleaseAssets} disabled={isProcessing}>
